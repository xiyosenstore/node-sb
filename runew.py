        ...
        try:
            with tarfile.open(tmp, "r:gz") as tar:
                ...
                if not member:
                    ...
                    fatal(f"В архиве нет бинарника sing-box. Примеры: {sample}")
                with tar.extractfile(member) as fsrc:
                    if fsrc is None:
                        fatal("Не удалось извлечь sing-box")
...
# ----------------- Client URL -----------------
def generate_client_url(return_str=False):
...
class SubHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/sub":
            url = generate_client_url(return_str=True)
...
def run_webserver(port=80):
    ...
    info(f"Web сервер запущен на http://0.0.0.0:{port}/sub")
